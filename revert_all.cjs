...
console.log("Reverting App.jsx...");
let appJsx = fs.readFileSync('src/App.jsx', 'utf8');
appJsx = appJsx.replace('h-full md:h-full', 'h-[60%] md:h-full');
fs.writeFileSync('src/App.jsx', appJsx);

console.log("Reverting ActiveRoute.jsx...");
let activeRouteJsx = fs.readFileSync('src/components/ActiveRoute/ActiveRoute.jsx', 'utf8');
activeRouteJsx = activeRouteJsx.replace(/h-full md:h-full/g, "${viewMode === 'map' ? 'h-[75%]' : 'h-[60%]'} md:h-full");
fs.writeFileSync('src/components/ActiveRoute/ActiveRoute.jsx', activeRouteJsx);

console.log("Reverting index.css...");
let indexCss = fs.readFileSync('src/index.css', 'utf8');
const mapCustomStylesIndex = indexCss.indexOf('/* MAP CUSTOM STYLES */');
if (mapCustomStylesIndex !== -1) {
    indexCss = indexCss.substring(0, mapCustomStylesIndex);
    fs.writeFileSync('src/index.css', indexCss);
}

console.log("Reverting StartPoint.jsx...");
...
if (startPointJsx.includes(newLayout)) {
    startPointJsx = startPointJsx.replace(newLayout, oldLayout);
    // Also revert end point layout class
    startPointJsx = startPointJsx.replace('bg-brand-input rounded-[10px] h-[35px]', 'bg-brand-input rounded-[16px] h-[52px]');
    fs.writeFileSync('src/components/StartPoint/StartPoint.jsx', startPointJsx);
    console.log("StartPoint.jsx reverted.");
} else {
    console.log("StartPoint.jsx new layout not found.");
}
