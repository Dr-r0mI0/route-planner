...
export default function StartPoint() {
  const { startPoint, setStartPoint, endPoint, setEndPoint, roundTrip, setRoundTrip, preferredTypes, setPreferredTypes } = useApp();
  const { t } = useI18n();
  ...
  const activities = [
    { id: 'pharmacy', label: t('activity.pharmacy') },
    { id: 'clinic', label: t('activity.clinic') },
    { id: 'hospital', label: t('activity.hospital') }
  ];

  const toggleActivity = (id) => {
    if (preferredTypes.includes(id)) {
      setPreferredTypes(preferredTypes.filter(t => t !== id));
    } else {
      setPreferredTypes([...preferredTypes, id]);
    }
  };

  useEffect(() => {
    if (startPoint && !inputValue) {
      setInputValue(`${startPoint.lat.toFixed(6)}, ${startPoint.lng.toFixed(6)}`);
      setGpsStatus('success');
    }
  }, [startPoint]);

  useEffect(() => {
    if (endPoint && !endInputValue) {
      setEndInputValue(`${endPoint.lat.toFixed(6)}, ${endPoint.lng.toFixed(6)}`);
    }
  }, [endPoint]);

  const handleGps = useCallback(() => {
    if (!navigator.geolocation) { setGpsStatus('error'); setGpsError(t('start.gps.unsupported')); return; }
    setGpsStatus('loading'); setGpsError('');
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const point = { lat: pos.coords.latitude, lng: pos.coords.longitude, displayName: 'My Location' };
        setStartPoint(point);
        setInputValue(`${point.lat.toFixed(6)}, ${point.lng.toFixed(6)}`);
        setGpsStatus('success');
      },
      (err) => {
        setGpsStatus('error');
        setGpsError(err.code === 1 ? t('start.gps.denied') : err.code === 2 ? t('start.gps.unavailable') : t('start.gps.timeout'));
      },
  ...
  const handleInputChange = useCallback((e) => {
    ...
    if (coords) { setStartPoint({ lat: coords.lat, lng: coords.lng, displayName: 'Custom Start' }); setGpsStatus('success'); }
    else if (val.trim() === '') { setStartPoint(null); setGpsStatus(null); }
  }, [setStartPoint]);
  ...
  const handleEndInputChange = useCallback((e) => {
    ...
    const coords = extractCoordsFromUrl(val);
    if (coords) { setEndPoint({ lat: coords.lat, lng: coords.lng, displayName: 'Custom End' }); }
    else if (val.trim() === '') { setEndPoint(null); }
  }, [setEndPoint]);
  ...
  return (
    <>
      ...
      <label className="flex items-center gap-3 ml-2 mt-1 cursor-pointer select-none group w-max mb-1">
        <input type="checkbox"
      ...
      {/* End Point Input */}
      {!roundTrip && (
        <div className="bg-brand-input rounded-[10px] h-[35px] flex items-center px-4 w-full border border-transparent focus-within:border-brand-orange/50 transition-colors">
          <input type="text"
            className="w-full bg-transparent border-none outline-none font-archivo text-sm text-brand-text tracking-[0.1em] placeholder-gray-400 uppercase opacity-90 font-[800]"
            placeholder={t('end.label')} autoComplete="off" name="end_point"
            value={endInputValue} onChange={handleEndInputChange} />
        </div>
      )}

      {/* Activity Type Dropdown */}
      <div className="w-full relative shrink-0 mt-2">
        <button
          type="button"
